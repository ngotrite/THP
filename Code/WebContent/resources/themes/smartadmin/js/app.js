 ...
 */
function showPwFtp(){
	$('.inputPasswordFtp').attr('type', 'text');
	$('.btn-showPassword').toggleClass('hidden');
	$('.btn-hidePassword').toggleClass('hidden');
}
	
function hidePwFtp(){
	$('.inputPasswordFtp').attr('type', 'password');
	$('.btn-showPassword').toggleClass('hidden');
	$('.btn-hidePassword').toggleClass('hidden');
}
...
function clearAppCache() {
	sessionStorage.removeItem('cacheMenuLeft');
	sessionStorage.removeItem('minified');
	sessionStorage.removeItem('hidden-menu');
}
...
$(document).ready(function() {
	...
	if (sessionStorage.getItem('cacheMenuLeft') !== null) {
		$('#myTab').html(sessionStorage.getItem('cacheMenuLeft'));
		sessionStorage.removeItem('cacheMenuLeft');
		
		if(sessionStorage.getItem('minified') !== null){
		...
		if(sessionStorage.getItem('hidden-menu') !== null){
			if(sessionStorage.getItem('hidden-menu') === 'true'){
	...
	$('#myTab a').click(function(e) {
		var menuLeftAll = $('#myTab li');
		for(i = 0; i < menuLeftAll.length; i++ ){
			$(menuLeftAll[i]).removeClass('active');
		}
		$(this).parent().addClass('active');
		sessionStorage.setItem('cacheMenuLeft', $('#myTab').html());
		if($('body').hasClass('minified')){
			sessionStorage.setItem('minified', true);
		}
		
		if($('body').hasClass('hidden-menu')){
			sessionStorage.setItem('hidden-menu', true);
		}
	});
	
	// huannn - trick replace url encode
	var AllMenus = $('a');
	for(var ii = 0; ii < AllMenus.length; ii++) {
		var hrefMenu = $(AllMenus[ii]).attr('href');
		if(hrefMenu && hrefMenu.includes('javax.faces.Token=')) {
			$(AllMenus[ii]).attr('href', hrefMenu.replace(/%25/g,'%'));			
		}
	}
	...
	$(".text-field-only-number").keydown(function (e) {
        ...
        if ((e.shiftKey || (e.keyCode == 173 || e.keyCode == 109))) {
	...
	// Collapse slideup define panel
	$('.btn-slideup-define').click(function(e) {
		if($('.box-panel-define button i.fa').hasClass('fa-angle-down')){
			$(".box-panel-define .box-body").slideUp();
			$('.box-panel-define button i.fa').removeClass('fa-angle-down').addClass('fa-angle-up');
		} else if ($('.box-panel-define button i.fa').hasClass('fa-angle-up')){
			$(".box-panel-define .box-body").slideDown();
			$('.box-panel-define button i.fa').removeClass('fa-angle-up').addClass('fa-angle-down');
		}
	});
	
	// COLLAPSE CommonTree
	$('.minimumTree').click(function(e) {
		if($('button i.fa').hasClass('fa-angle-left')){
			$('.heightTree').animate({ height: '0px' }, 500, function() {
				$(".treeViewData").addClass("hidden");
				$('.treeContainer .box-title-tree').addClass("hidden");
				$('.treeContainer').animate({ width: '15px' }, 500, function() {
		        });
		        $('.contentTreeContainer').animate({ width: '98%' }, 500, function() {
		        });
		        $('button i.fa').removeClass('fa-angle-left').addClass('fa-angle-right');
	        });
		} else if ($('button i.fa').hasClass('fa-angle-right')){
			$('.treeContainer').animate({ width: '33.33333333333333%' }, 500, function() {
				$('.treeContainer .box-title-tree').removeClass("hidden");
				$(".treeViewData").removeClass("hidden");
				$('.heightTree').animate({ height: $(".heightContentTree").height() }, 500, function() {
		        });
				
	        });
			$('.contentTreeContainer').animate({ width: '66.6666666666666666%' }, 500, function() {
	        });
	        $('button i.fa').removeClass('fa-angle-right').addClass('fa-angle-left');
		}
		e.preventDefault();
	});
	...
	*/
	...
	$(document).mouseup(function(e) {
		if ($.shortcut_dropdown != undefined && !$.shortcut_dropdown.is(e.target)// if the target of the click isn't the container...
	...
	jarvismenu : function(options) {
		...
		$this.find("li").each(function() {
			if ($(this).find("ul").size() != 0) {
				...
				var fisrt = $(this).find("a:first").html();
				if(fisrt.indexOf('<b class="collapse-sign">') === -1){
		...
		$this.find("li.active").each(function() {
			...
			$(this).parents("ul").parent("li").addClass("open")
                        $(this).parents("ul").parent("li").addClass("hightline")
		});
...
notifyNew = {
		...
		showNotification: function(from, align, type, textHead, textContent){
	    	color = Math.floor((Math.random() * 4) + 1);
	    	
	    	$.notify({
	        	icon: "pe-7s-gift",
	        	message: "<b>" + textHead + "</b>" + " " + textContent, 
