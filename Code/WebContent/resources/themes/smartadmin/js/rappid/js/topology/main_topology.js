var Rappid = Backbone.Router.extend({
    ...
    toTextView: function() {
       $('#json_containter textarea').val(JSON.stringify(this.graph.toJSON()));
         
    },
    renderJson: function() {
        var val = $('#json_containter textarea').val();  
        if(val && val != "null") {                    
            var jsonInput = JSON.parse(val);            
            this.graph.fromJSON(jsonInput);
        }
    ...
    initializePaper: function() {
        ...
        this.paper = new joint.dia.Paper({
            ...
            validateConnection: function(cellViewS, magnetS, cellViewT, magnetT, end, linkView) {
                var sourceLink = cellViewS.model.attributes.cusType;
                var targetLink = cellViewT.model.attributes.cusType;
                
                // initSpout only connect initBolt
                if(sourceLink == "initSpout" && targetLink != "initBolt") {
                    if(linkView != undefined) {
                        linkView.remove();
                    }
                    return false;                             
                }
                // init Spout recived connect comment, mes
                else if(sourceLink != "initSpout" && targetLink == "initSpout") {                                
                    if(sourceLink != "comment" && sourceLink != "message") {
                        if(linkView != undefined) {
                            linkView.remove();
                        }
                        return false;    
                ...
                // Bolt init only connect simple bolt
                else if(sourceLink == "initBolt" && targetLink != "simpleBolt") {
                    if(linkView != undefined) {
                        linkView.remove();
                    }
                    return false;                             
                } 
                // Bolt init recive connect comment, mes,Spout init
                else if(sourceLink != "initBolt" && targetLink == "initBolt") { 
                    if(sourceLink != "initSpout" && sourceLink != "comment" && sourceLink != "message") {
                        if(linkView != undefined) {
                            linkView.remove();
                        }
                        return false; 
                ...
                // Simple Bolt only recived connect Bolt init, comment, mes
                else if(targetLink == "simpleBolt") { 
                    if(sourceLink != "simpleBolt" && sourceLink != "initBolt" && sourceLink != "comment" && sourceLink != "message") {
                        if(linkView != undefined) {
                            linkView.remove();
                        }
                        return false; 
                ...
                // Spout not connect Spout
                if(sourceLink == "simpleSpout" && targetLink == "simpleSpout") {                                        
                    if(linkView != undefined) {
                        linkView.remove();
                    }
                    return false;                    
    ...
    initializeLinkTooltips: function(cell) {
        ...
        if (cell instanceof joint.dia.Link) {
            ...
            new joint.ui.Tooltip({
                className: 'tooltip small bachtx3',
    ...
    initializeStencil: function() {
        ...
        this.stencil = new joint.ui.Stencil({
            ...
            search: {
                '*': ['type','attrs/text/text','attrs/.label/text','attrs/.name/text', 'attrs/.remark/text','content'],
        ...
        _.each(Stencil.groups, function(group, name) { 
            switch(name) {
                case "Bolt":
                    ...
                    joint.layout.GridLayout.layout(this.stencil.getGraph(name), layoutOptionsBolt);
        ...
        _.each(this.stencil.graphs, function(graph) {
            ...
            graph.get('cells').each(function(cell) {
                ...
                new joint.ui.Tooltip({
                    ...
                    content: cell.get('tooltips').split('.').join(' '),
                    left: '.stencil',
                    direction: 'left'
                });
            });
    ...
    createInspector: function(cellView) {
        ...
        // if (!this.inspector || this.inspector.options.cell !== cell) {
            ...
            var cusType = cell.get('cusType');

            if(cusType == "initSpout") {
                var inspectorDefs = InspectorDefs["basic.Circle_Init"];

            } else if(cusType == "initBolt") {
                var inspectorDefs = InspectorDefs["basic.Rect_Init"];
        ...
        //@author bachtx3
        this.inspector.$('select[data-attribute="selected_function"]').on('change', function() {
            self.addClassFirstFunc();                    
            if($('.func_name').length > 0)
            {            
                var value = self.inspector.$('select[data-attribute="selected_function"] option:selected').text(); 
                if(value == "Choose functions") {
                    value = "my function";
                }
                $(".func_name input").val(value);
            } 
            
            self.createInspector(cellView);            
        });

        // this.inspector.$('select[data-attribute="value"]').on('change', function() {            
        //     var value = self.inspector.$('select[data-attribute="value"]').val();            
        //     self.inspector.$('textarea[data-attribute="attrs/text/text"]').html(value);            
        // }); 
         // chose function in condistions
        // this.inspector.$('select[data-attribute="choose_function"]').on('change', function() {            
            // var value = self.inspector.$('select[data-attribute="choose_function"]').val();
            // self.createInspector(cellView, "cd_func");
        // });     
    },
    ...
    initializeHaloAndInspector: function() {
        ...
        this.paper.on('cell:pointerup', function(cellView, evt) {
            ...
            //bachtx3
            if(cellView.model.attributes.type == "bpmn.Group") {
        ...
        this.paper.on('link:options', function(evt, cellView, x, y) {                        
            ...
            var idSrc = cellView.model.attributes.source.id;
            var idTarget = cellView.model.attributes.target.id;
            var sourceObj;
            var targetObj;
            if(idTarget && idSrc) {
                var listItem = this.graph.toJSON().cells;
                for(var i =0; i< listItem.length; i++){
                    var item = listItem[i];
                    if(item.id == idSrc){
                        sourceObj = item;
                    }else if(item.id == idTarget){
                        targetObj = item;
                    }
                }

                if((sourceObj.cusType == "simpleBolt" || sourceObj.cusType == "initBolt") && targetObj.cusType == "simpleBolt") {                    
                    $('div[data-field="is_Use_Stream"]').css("display","block");

                } else {                    
                    $('div[data-field="is_Use_Stream"]').css("display","none");
                }
    ...
    initializeToolbar: function() {
        ...
        $('#fullscreen').on('click', _.bind(this.toggleFullscreen, this));
    ...
    // @author bachtx3
    addClassFirstFunc: function() {
        if($('.list-items').length > 0)
        {            
            $(".list-items").find(".list-item:first").addClass("func_name");            
        }         
    },
    setAttrReadOnlyInput: function(){            
         $('input[data-attribute="effectDate"]').attr('readonly', true);
         $('input[data-attribute="attrs/.name/text"]').attr('readonly', true);
    }
